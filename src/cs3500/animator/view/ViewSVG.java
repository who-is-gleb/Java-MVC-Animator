...
public class ViewSVG implements ExcellenceView {
  ...
  public void setTickRate(int rate) {
    //TODO: you need to get this tickRate and use it to calculate times in the svg!
  }
  ...
  public void startView(PrintStream out) {
    out.append(
        "<svg width=\"" + canvasInfo[2] + "\" height=\"" + canvasInfo[3] + "\" version=\"1.1\" "
          ...
          for (int i = 0; i < s.changes.size(); i++) {
            for (int k = 1; k < (s.changes.get(i).size() / 2); k++) {
              if (!s.changes.get(i).get(k).equals(s.changes.get(i).get(k + 8))) {
                out.append(
                    "    <animate attributeType=\"xml\" begin=\"" + s.changes.get(i).get(0)
                        + "ms\" dur=\""
                        + (s.changes.get(i).get(8) - s.changes.get(i).get(0))
                        + "ms\" attributeName=\"");
                if (k == 1) {
                  out.append("x\" from=\"" + s.changes.get(i).get(k) + "\" to=\"" + s.changes.get(i)
                      .get(k + 8)
                      + "\" fill=\"freeze\" />" + "\n");
                } else if (k == 2) {
                  out.append("y\" from=\"" + s.changes.get(i).get(k) + "\" to=\"" + s.changes.get(i)
                      .get(k + 8)
                      + "\" fill=\"freeze\" />" + "\n");
                } else if (k == 3) {
                  out.append("w\" from=\"" + s.changes.get(i).get(k) + "\" to=\"" + s.changes.get(i)
                      .get(k + 8)
                      + "\" fill=\"freeze\" />" + "\n");
                } else if (k == 4) {
                  out.append("h\" from=\"" + s.changes.get(i).get(k) + "\" to=\"" + s.changes.get(i)
                      .get(k + 8)
                      + "\" fill=\"freeze\" />" + "\n");
                } else if (k == 5) {
                  out.append("r\" from=\"" + s.changes.get(i).get(k) + "\" to=\"" + s.changes.get(i)
                      .get(k + 8)
                      + "\" fill=\"freeze\" />" + "\n");
                } else if (k == 6) {
                  out.append("g\" from=\"" + s.changes.get(i).get(k) + "\" to=\"" + s.changes.get(i)
                      .get(k + 8)
                      + "\" fill=\"freeze\" />" + "\n");
                } else if (k == 7) {
                  out.append("b\" from=\"" + s.changes.get(i).get(k) + "\" to=\"" + s.changes.get(i)
                      .get(k + 8)
                      + "\" fill=\"freeze\" />" + "\n");
                }
              }
          ...
          for (int j = 0; j < s.changes.size(); j++) {
            for (int f = 1; f < (s.changes.get(j).size() / 2); f++) {
              if (!s.changes.get(j).get(f).equals(s.changes.get(j).get(f + 8))) {
                out.append(
                    "    <animate attributeType=\"xml\" begin=\"" + s.changes.get(j).get(0)
                        + "ms\" dur=\""
